...
class MushroomEncyclopediaTab(
    ...
    companion object {
        ...
        private const val MAX_ONLINE_FETCH_LOOPS = 6
    }
    ...
    private var onlineApiPage = 1
    private val onlineBuffer = mutableListOf<MushroomTaxon>()
    private val seenScientificNames = mutableSetOf<String>()
    private val displayedTaxa = mutableListOf<MushroomTaxon>()
    private var isLoading = false
    private var hasMore = true
    private var hasMoreApi = true
    ...
    init {
        ...
        val savedEdibility = AppPrefs.getMushroomEdibilityFilter(activity)
        currentEdibilityFilter = when (savedEdibility) {
            "edible", "inedible" -> savedEdibility
            else -> "all"
        }
        val savedHymenium = AppPrefs.getMushroomHymeniumFilter(activity)
        currentHymeniumFilter = when (savedHymenium) {
            "tubes", "gills", "other" -> savedHymenium
            else -> "all"
        }
    ...
    private fun getEdibilityTitle(key: String): String {
        return when (key) {
            ...
            "inedible" -> if (currentLang == "uk") "🔴 Неїстівні" else "🔴 Inedible"
    ...
    private fun getHymeniumTitle(key: String): String {
        return when (key) {
            ...
            "other" -> if (currentLang == "uk") "🍄 Інші" else "🍄 Other"
    ...
    private fun showFilterDialog() {
        ...
        val edibilityOptions = if (currentLang == "uk") {
            listOf(
                Triple("all", "📋 Всі види", "Повний каталог грибів без обмежень за категоріями"),
                Triple("edible", "🟢 Їстівні", "Безпечні для споживання та кулінарно цінні види"),
                Triple("inedible", "🔴 Неїстівні", "Отруйні, токсичні, галюциногенні, умовно-їстівні та невідомі")
            )
        } else {
            listOf(
                Triple("all", "📋 All species", "Complete catalog with no category restrictions"),
                Triple("edible", "🟢 Edible", "Safe and culinary approved edible species"),
                Triple("inedible", "🔴 Inedible", "Poisonous, toxic, hallucinogenic, inedible and unknown")
            )
        }
        ...
        for ((key, title, subtitle) in edibilityOptions) {
            val ssb = SpannableStringBuilder().apply {
                append(title)
                setSpan(StyleSpan(Typeface.BOLD), 0, length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
                setSpan(AbsoluteSizeSpan((14 * density).toInt()), 0, length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
                setSpan(ForegroundColorSpan(Color.WHITE), 0, length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)

                append("\n")
                val startSub = length
                append(subtitle)
                setSpan(AbsoluteSizeSpan((11.5f * density).toInt()), startSub, length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
                setSpan(ForegroundColorSpan(Color.parseColor("#9CA3AF")), startSub, length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
            }
            ...
            val rb = RadioButton(activity).apply {
                ...
                setPadding((8 * density).toInt(), (4 * density).toInt(), (8 * density).toInt(), (6 * density).toInt())
        ...
        val hymeniumOptions = if (currentLang == "uk") {
            listOf(
                Triple("all", "📋 Всі типи", "Усі гриби незалежно від форми спороносного шару"),
                Triple("tubes", "🧽 Трубчасті", "Пориста губка під шапинкою (боровики, маслюки, підберезники)"),
                Triple("gills", "🍂 Пластинчасті", "Радіальні пластинки (печериці, сироїжки, опеньки, мухомори)"),
                Triple("other", "🍄 Інші", "Дощовики, їжовики, лисички, трутовики, строчки тощо")
            )
        } else {
            listOf(
                Triple("all", "📋 All types", "All mushroom types regardless of spore structure"),
                Triple("tubes", "🧽 Tubes (Pores)", "Porous sponge under cap (boletes, suillus, leccinum)"),
                Triple("gills", "🍂 Gilled", "Radial gills (agarics, russula, amanita, honey fungi)"),
                Triple("other", "🍄 Other", "Puffballs, tooth fungi, chanterelles, bracket fungi, etc.")
            )
        }
        ...
        for ((key, title, subtitle) in hymeniumOptions) {
            val ssb = SpannableStringBuilder().apply {
                append(title)
                setSpan(StyleSpan(Typeface.BOLD), 0, length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
                setSpan(AbsoluteSizeSpan((14 * density).toInt()), 0, length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
    ...
    private fun promptAndDownloadDatabase(onReady: ((Boolean) -> Unit)? = null) {
        ...
        val msg = if (isUpdate) {
            if (currentLang == "uk") {
                "Розмір встановленої бази не співпадає з актуальною версією (538 МБ).\n\n" +
                "Оновлення виправляє дублювання фотографій та містить актуалізовані описи видів.\n\n" +
                "Оновити базу даних зараз?"
            } else {
                "Installed database size differs from current version (538 MB).\n\n" +
                "The update fixes duplicate photos and contains refreshed descriptions.\n\n" +
                "Update database now?"
            }
        } else {
            if (currentLang == "uk") {
                "Для роботи енциклопедії без інтернету потрібна локальна база даних (~538 МБ). Завантажити зараз?"
            } else {
                "Offline mushroom encyclopedia requires local database (~538 MB). Download now?"
            }
        }
    ...
    private fun resetAndReload() {
        ...
        offlinePage = 1
        onlineApiPage = 1
        synchronized(onlineBuffer) {
            onlineBuffer.clear()
            seenScientificNames.clear()
        }
        displayedTaxa.clear()
    ...
    private fun loadNextPage() {
        ...
        val hasActiveFilter = (currentEdibilityFilter != "all" || currentHymeniumFilter != "all")
        statusTextView.text = if (isOfflineMode) {
            if (currentLang == "uk") "Завантаження з локальної бази..." else "Loading from local database..."
        } else {
            if (hasActiveFilter && MushroomDatabaseManager.isDatabaseAvailable(activity)) {
                if (currentLang == "uk") "Завантаження з каталогу..." else "Loading catalog..."
            } else {
        ...
        if (isOfflineMode) {
            ...
            Thread {
                ...
                mainHandler.post {
                    if (requestGen != searchGeneration) return@post
                    isLoading = false
                    offlinePage++
                    hasMore = moreAvailable
                    appendTaxaViews(taxa)
                    displayedTaxa.addAll(taxa)

                    if (displayedTaxa.isEmpty()) {
                        statusTextView.visibility = View.VISIBLE
                        statusTextView.text = if (currentLang == "uk") {
                            "Грибів за обраним фільтром не знайдено"
                        } else {
                            "No species found matching the selected filter"
                        }
                    } else {
        ...
        // Hybrid mode: if filters are active and offline database is available, use database catalog
        // to deliver instant, complete, 24-per-page results identically to offline,
        // while remaining in online mode (online detail dialog on click, etc.)
        if (hasActiveFilter && MushroomDatabaseManager.isDatabaseAvailable(activity)) {
            Thread {
                val (taxa, moreAvailable) = MushroomDatabaseManager.queryTaxa(
                    context = activity,
        ...
        // Online mode (without filters, or fallback if database not yet downloaded):
        // accumulate matching taxa into onlineBuffer until PAGE_SIZE is reached or API exhausted
        Thread {
            val query = currentQuery
            val lang = currentLang
            val hymeniumFilter = currentHymeniumFilter
            val fetchBatchSize = if (hasActiveFilter) 200 else 100
            var loops = 0

            while (hasMoreApi && loops < MAX_ONLINE_FETCH_LOOPS) {
                if (requestGen != searchGeneration) return@Thread
                val currentBufferSize = synchronized(onlineBuffer) { onlineBuffer.size }
                if (currentBufferSize >= PAGE_SIZE) break

                loops++
                val (rawTaxa, apiMore) = if (query.isNotEmpty()) {
                    MushroomApiClient.searchTaxaSync(query, lang, onlineApiPage, perPage = 60, hymeniumFilter = hymeniumFilter)
                } else {
                    MushroomApiClient.getPopularTaxaSync(false, lang, onlineApiPage, perPage = fetchBatchSize, hymeniumFilter = hymeniumFilter)
                }

                hasMoreApi = apiMore && rawTaxa.isNotEmpty()
                onlineApiPage++

                synchronized(onlineBuffer) {
                    for (t in rawTaxa) {
                        val key = t.scientificName.lowercase().trim()
                        if (seenScientificNames.add(key)) {
                            if (matchesFilter(t)) {
                                onlineBuffer.add(t)
                            }
                        }
                ...
                if (!hasMoreApi || rawTaxa.isEmpty()) break
            }
            ...
            mainHandler.post {
                ...
                val batch = synchronized(onlineBuffer) {
                    val countToTake = minOf(PAGE_SIZE, onlineBuffer.size)
                    if (countToTake > 0) {
                        val sub = onlineBuffer.subList(0, countToTake).toList()
                        onlineBuffer.subList(0, countToTake).clear()
                        sub
                    } else {
                        emptyList()
                    }
                ...
                val remainingBuffer = synchronized(onlineBuffer) { onlineBuffer.size }
                hasMore = remainingBuffer > 0 || hasMoreApi

                if (batch.isNotEmpty()) {
                    appendTaxaViews(batch)
                    displayedTaxa.addAll(batch)
                }
                ...
                // If screen is still not filled enough to scroll and more items are available, trigger next page
                if (hasMore && displayedTaxa.size < 12) {
                    mainHandler.postDelayed({
                        if (hasMore && !isLoading && displayedTaxa.size < 12 && requestGen == searchGeneration) {
                            loadNextPage()
                        }
                    }, 200)
                }
    ...
    private fun matchesFilter(taxon: MushroomTaxon): Boolean {
        ...
        val hymenium = if (meta.hymenium != "other") meta.hymenium else if (!taxon.hymenium.isNullOrBlank()) taxon.hymenium else "other"

        val matchesEdibility = when (currentEdibilityFilter) {
            "all" -> true
            "edible" -> edibility == "edible"
            "inedible" -> edibility != "edible"
            else -> true
        }
    ...
    private fun appendTaxaViews(taxaToAppend: List<MushroomTaxon>) {
        if (taxaToAppend.isEmpty()) return
        ...
        if (currentViewMode == "grid") {
            ...
            if (itemsContainer.childCount > 0) {
                val lastRow = itemsContainer.getChildAt(itemsContainer.childCount - 1) as? LinearLayout
                if (lastRow != null && lastRow.childCount == 2) {
                    val secondChild = lastRow.getChildAt(1)
                    if (secondChild.tag != "grid_card") {
                        lastRow.removeViewAt(1)
                        val card = createGridCard(taxaToAppend[0]).apply { tag = "grid_card" }
            ...
            for (i in startIndex until taxaToAppend.size step 2) {
                ...
                val card1 = createGridCard(taxaToAppend[i]).apply { tag = "grid_card" }
                row.addView(card1)
                if (i + 1 < taxaToAppend.size) {
                    val card2 = createGridCard(taxaToAppend[i + 1]).apply { tag = "grid_card" }
                    row.addView(card2)
                } else {
            ...
            for (taxon in taxaToAppend) {
                renderListCard(taxon)
            }
        }
    }

    private fun reRenderDisplayed() {
        itemsContainer.removeAllViews()
        val current = displayedTaxa.toList()
        displayedTaxa.clear()
        appendTaxaViews(current)
        displayedTaxa.addAll(current)
    }
    ...
    private fun createGridCard(taxon: MushroomTaxon): View {
        ...
        val targetPhotoUrl = taxon.defaultPhotoUrl
        ivPhoto.tag = targetPhotoUrl
        MushroomApiClient.loadBitmap(targetPhotoUrl) { bmp ->
            if (bmp != null && ivPhoto.tag == targetPhotoUrl) ivPhoto.setImageBitmap(bmp)
        }
        ...
        val hymeniumIcon = MycoKnowledge.getHymeniumIcon(effectiveHymenium)
    ...
    private fun renderListCard(taxon: MushroomTaxon) {
        ...
        val targetPhotoUrl = taxon.defaultPhotoUrl
        thumbIv.tag = targetPhotoUrl
        MushroomApiClient.loadBitmap(targetPhotoUrl) { bmp ->
            if (bmp != null && thumbIv.tag == targetPhotoUrl) thumbIv.setImageBitmap(bmp)
        }
