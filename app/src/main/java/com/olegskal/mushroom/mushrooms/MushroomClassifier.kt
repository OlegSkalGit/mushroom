    ...
    companion object {
        ...
        fun downloadModel(
            ...
            Thread {
                ...
                if (forceDownload && zipFile.exists()) {
                    zipFile.delete()
                } else if (zipFile.exists() && zipFile.length() > MODEL_ZIP_FULL_SIZE) {
                    zipFile.delete()
                }

                var downloadSuccess = (zipFile.exists() && zipFile.length() == MODEL_ZIP_FULL_SIZE)
                var lastError: String? = null
              ...
              <script>
                ...
                async function classifyBatchBase64(base64Data, batchSize) {
                  try {
                    ...
                    AndroidBridge.onInferenceResult(JSON.stringify(allOutputs), batchSize);
                  } catch(e) {
